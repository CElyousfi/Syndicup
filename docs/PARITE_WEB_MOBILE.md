...
| M20 | Dossier RH d'un employé : fiche (poste, contrat, salaire, CNSS masqué, horaires, contact d'urgence, documents), fiches de paie + PDF FR / AR, congés (demande, annulation, décision syndic), présences (mois) | ✓ (`personnel/[id]`, onglets) | ✓ (`/personnel/:id`, onglets Fiche / Paie / Congés / Présences) | proposé | Même API `/personnel/*`, mêmes dictionnaires (`personnel.*`, `enumsPersonnelRh.*`) ; pushs `CONGE_*` / `PAIE_VALIDEE` / `CONTRAT_TRAVAIL_FIN_PROCHE` → ces écrans. |
| M20 | « Mon dossier » de l'employé + pointage de présence | ✓ (`personnel/me`, bouton « Je suis présent ») | ✓ (`/personnel/me`, pointage **hors-ligne** : file Drift `presences_queue`, Idempotency-Key = id de ligne) | proposé | Le pointage se fait à la loge : le mobile est premier (file locale, rejeu au retour du réseau) ; le web pointe en ligne. |
| M20 | Planning hebdomadaire (horaires, congés approuvés + remplaçants, présences) | ✓ (`personnel/planning`) | ✓ (`/personnel/planning`) | proposé | Même API `GET /personnel/planning?semaine=`. |
| M20 | Modification du dossier RH, paramètres de paie, préparation / validation / paiement des fiches, paie du mois, saisie en masse des présences, évaluations, exports csv / xlsx | ✓ | ✗ (lecture : fiches + PDF, présences ; la validation de paie et l'évaluation restent web) | proposé | Gestes administratifs syndic (règle du prompt : écrans d'administration web-first) ; le syndic approuve / refuse les congés depuis le mobile. |
