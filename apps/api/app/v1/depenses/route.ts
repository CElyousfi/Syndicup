 ...
 * tri, `?format=csv` (export journalisé). POST : BROUILLON (syndic).
 */
import { withApiHandler } from "../../../lib/http/handler";
import { tenantFromRequest } from "../../../lib/http/request-context";
import { ok, failZod } from "../../../lib/http/respond";
import { lirePagination, lireTri, metaPagination } from "../../../lib/http/pagination";
import { formatDemande, reponseCsv } from "../../../lib/http/export";
...
async function handleGET(req: Request) {
  try {
    ...
    if (formatDemande(url) === "csv") {
      const { entetes, lignes } = await exporterDepensesCsv(ctx, parsed.data);
      return reponseCsv(`depenses-${parsed.data.exercice ?? "toutes"}.csv`, entetes, lignes);
    }
