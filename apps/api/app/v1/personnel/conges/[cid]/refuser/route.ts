/** POST /v1/personnel/conges/{cid}/refuser — décision du syndic (approuver / refuser, Idempotency-Key) ou annulation (employé pour sa demande, syndic). */
...
async function handlePOST(req: Request, { params }: { params: Promise<{ cid: string }> }) {
  try {
    ...
    return ok(await deciderConge(ctx, cid, "REFUSE", parsed.data, readIdempotencyKey(req)));
